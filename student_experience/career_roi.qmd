...
viewof roi_stipend = {
  ...
  const nearest = options.reduce((a,b) => Math.abs(b-(isFinite(roi_stipend_init)?roi_stipend_init:30000))<Math.abs(a-(isFinite(roi_stipend_init)?roi_stipend_init:30000))?b:a, 30000);
  const el = Inputs.select(options, {
    label: "Annual Stipend (pre-tax, $) — override updates other pages",
...
viewof roi_duration = {
  ...
  const nearest = opts.reduce((a,b) => Math.abs(b-(isFinite(roi_duration_init)?roi_duration_init:6))<Math.abs(a-(isFinite(roi_duration_init)?roi_duration_init:6))?b:a, 6);
  const el = Inputs.select(opts, {
    label: "PhD Duration (years) — override updates other pages",
    format: d => `${d} years`,
    value: nearest
  });
...
```{ojs}
//| echo: false
viewof roi_alt_salary = {
  const saved = parseFloat(localStorage.getItem('phd_alt_salary') || '70000');
  const options = [40000,45000,50000,55000,60000,65000,70000,75000,80000,85000,90000,95000,100000,110000,120000,130000,140000,150000];
  const nearest = options.reduce((a,b) => Math.abs(b-(isFinite(saved)?saved:70000))<Math.abs(a-(isFinite(saved)?saved:70000))?b:a, 70000);
  const el = Inputs.select(options, {
    label: "Alternative Career Salary (BS/MS entry-level, pre-tax, $) — what you'd earn without a PhD",
    format: d => `$${d.toLocaleString()}`,
    value: nearest
  });
  el.addEventListener('change', () => localStorage.setItem('phd_alt_salary', String(el.value)));
  return el;
}
```
...
```{ojs}
//| echo: false
viewof roi_target_sector = Inputs.select(roi_sector_data, {
  label: "Target Career Sector (post-PhD)",
  format: s => s.sector,
  value: roi_sector_data[0]
})
```

```{ojs}
//| echo: false
viewof roi_postdoc_years = Inputs.select([0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5], {
  label: "Postdoc Years (if planning a postdoc; 0 = none)",
  format: d => d === 0 ? "None" : `${d} years`,
  value: 0
})
```
