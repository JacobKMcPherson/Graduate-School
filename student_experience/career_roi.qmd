...
viewof roi_stipend = {
  const el = Inputs.range([15000, 60000], {
    ...
    step: 500,
    value: isFinite(roi_stipend_init) ? roi_stipend_init : 30000
  });
...
viewof roi_duration = {
  const el = Inputs.range([3, 9], {
    ...
    step: 0.5,
    value: isFinite(roi_duration_init) ? roi_duration_init : 6
  });
...
viewof roi_alt_salary = {
  ...
  const el = Inputs.range([40000, 150000], {
    label: "Alternative Career Salary (BS/MS entry-level, pre-tax, $) — what you'd earn without a PhD",
    step: 1000,
    value: isFinite(saved) ? saved : 70000
  });
  el.addEventListener('input', () => localStorage.setItem('phd_alt_salary', String(el.value)));
  return el;
}
```
...
```{ojs}
//| echo: false
viewof roi_postdoc_years = Inputs.range([0, 5], {
