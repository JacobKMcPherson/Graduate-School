...
viewof roi_controls = {
  ...
  function update() {
    ...
    if (sharedState?.set) {
      sharedState.set('phd_annual_stipend', value.stipend);
      sharedState.set('phd_duration', value.duration);
      sharedState.set('phd_alt_salary', value.alt_salary);
    } else {
  ...
  const syncFromShared = () => {
    let changed = false;
    const sharedStipend = nearest(stipendOptions, parseFloat((sharedState?.get('phd_annual_stipend', '30000')) || '30000'), 30000);
    const sharedDuration = nearest(durationOptions, parseFloat((sharedState?.get('phd_duration', '6')) || '6'), 6);
    const sharedAlt = nearest(altSalaryOptions, parseFloat((sharedState?.get('phd_alt_salary', '70000')) || '70000'), 70000);
    if (Number(stipendSelect.value) !== sharedStipend) {
      stipendSelect.value = String(sharedStipend);
      changed = true;
    }
    if (Number(durationSelect.value) !== sharedDuration) {
      durationSelect.value = String(sharedDuration);
      changed = true;
    }
    if (Number(altSalarySelect.value) !== sharedAlt) {
      altSalarySelect.value = String(sharedAlt);
      changed = true;
    }
    if (changed) update();
  };
  ...
  sharedState?.subscribe?.(['phd_annual_stipend', 'phd_duration', 'phd_alt_salary'], syncFromShared);
  update();
  return container;
}
```
